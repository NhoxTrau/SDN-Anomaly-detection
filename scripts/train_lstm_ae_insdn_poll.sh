...
CLASSIFIER_PREP_DIR="${SDN_NIDS_PREPARED_POLL_DIR:-artifacts_v4/prepared_poll_classifier_t${POLL_INTERVAL//./p}_${KEY_MODE}}"
...
if [ ! -f "$CLASSIFIER_PREP_DIR/train.npz" ] || [ "${SDN_NIDS_FORCE_PREPARE:-0}" = "1" ]; then
  $pybin -m sdn_nids_realtime.train_v2.poll_sequence_builder \
    --dataset-dir datasets/InSDN \
    --save-dir "$CLASSIFIER_PREP_DIR" \
...
$pybin -m sdn_nids_realtime.train_v2.run_train_autoencoder_poll \
  ...
  --source-prepared-dir "$CLASSIFIER_PREP_DIR" \
